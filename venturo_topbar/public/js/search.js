...
venturo_topbar.search.SearchDialog = class {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
		...
		this.searches = {
			global_search: {
				...
				get_results: (keywords, callback) => {
					let start = 0,
						limit = 100;
					let results = venturo_topbar.search.utils.get_nav_results(keywords);
					venturo_topbar.search.utils.get_global_results(keywords, start, limit).then(
						(global_results) => {
			...
			tags: {
				...
				get_results: (keywords, callback) => {
					var results = venturo_topbar.search.utils.get_nav_results(keywords);
	...
	render_data(result_sets) {
		// Use the search template HTML directly since template registration might not work
		const searchTemplate = `<div class="search-results flex">
		...
		let $search_results = $(searchTemplate).addClass("hide");
...
$(document).ready(function() {
	if (typeof frappe !== 'undefined') {
		venturo_topbar.searchdialog = {
			search: new venturo_topbar.search.SearchDialog()
		};
	}
});